...
type NWS struct {
	...
	matchOrdersCallback       func(order *WSMatchOrder)
...
func (ws *NWS) SubscribeOrders(id string, symbol string) {
	ws.SubscribeTopic(id,
		fmt.Sprintf("orders.%s", strings.ToLower(symbol)))
}

// SubscribeMatchOrders 订阅撮合订单成交数据
// symbol: BTC
func (ws *NWS) SubscribeMatchOrders(id string, symbol string) {
	ws.SubscribeTopic(id,
		fmt.Sprintf("matchOrders.%s", strings.ToLower(symbol)))
}
...
func (ws *NWS) SubscribeAccounts(id string, symbol string) {
	ws.SubscribeTopic(id,
		fmt.Sprintf("accounts.%s", strings.ToLower(symbol)))
}
...
func (ws *NWS) SetMatchOrdersCallback(callback func(order *WSMatchOrder)) {
	ws.matchOrdersCallback = callback
}
...
func (ws *NWS) handleMsg(messageType int, msg []byte) {
	...
	// 认证成功
	// {"op":"auth","type":"api","err-code":0,"ts":1586579120653,"data":{"user-id":"100726"}}

	// 订阅成功
	// {"op":"sub","cid":"40sG903yz80oDFWr","topic":"matchOrders.btc","ts":1586579121646,"err-code":0}

	// 下单事件
	// {"op":"notify","topic":"matchOrders.btc","ts":1586579194161,"symbol":"BTC","contract_type":"this_week","contract_code":"BTC200417","status":3,"order_id":698509296681648128,"order_id_str":"698509296681648128","order_type":1,"trade":[]}

	//  撤单事件
	// {"op":"notify","topic":"matchOrders.btc","ts":1586579226100,"symbol":"BTC","contract_type":"this_week","contract_code":"BTC200417","status":7,"order_id":698509296681648128,"order_id_str":"698509296681648128","order_type":2,"trade":[]}

	// 下单事件
	// {"op":"notify","topic":"orders.btc","ts":1586579446522,"symbol":"BTC","contract_type":"this_week","contract_code":"BTC200417","volume":2,"price":6200,"order_price_type":"limit","direction":"buy","offset":"open","status":3,"lever_rate":10,"order_id":698510354724319232,"order_id_str":"698510354724319232","client_order_id":null,"order_source":"web","order_type":1,"created_at":1586579446261,"trade_volume":0,"trade_turnover":0,"fee":0,"trade_avg_price":0.00,"margin_frozen":0.003225806451612903,"profit":0,"trade":[],"liquidation_type":"0"}

	// 撤单
	// {"op":"notify","topic":"orders.btc","ts":1586579526850,"symbol":"BTC","contract_type":"this_week","contract_code":"BTC200417","volume":2,"price":6200,"order_price_type":"limit","direction":"buy","offset":"open","status":7,"lever_rate":10,"order_id":698510354724319232,"order_id_str":"698510354724319232","client_order_id":null,"order_source":"web","order_type":2,"created_at":1586579446261,"trade_volume":0,"trade_turnover":0,"fee":0,"trade_avg_price":0.00,"margin_frozen":0E-18,"profit":0,"trade":[],"liquidation_type":"0"}
...
// NewNWS 创建 NWS
// wsURL:
// 正式地址 wss://api.hbdm.com/notification
// 开发地址 wss://api.btcgateway.pro/notification
